...
function general_condtion_load() {
    var _body = document.getElementById("general_body");
    $(_body).empty();

    var search_value_btn_close = document.getElementById("general_search_value_btn_close");
    var condition_search_value_btn = document.getElementById("general_condition_search_value_btn");

    var condition_search_value_text = document.getElementById("general_condition_search_value_text");
...
//                    alert(sending_value);
    $.ajax({
        url: "create_new_condition/list_general_condition.php",
        type: 'POST',
        data: sending_value,
        cache: false,
...
//                            alert(data);
            ...
            for (var i = 0; i < json.length; i++) {
                table_body(json[i].id, json[i].dis, _body, 2);
            }
...
function general_condtion_setup() {
    var header = document.getElementById("modal_head");
    $(header).empty();
    header.appendChild(document.createTextNode("Create New General Condition"));
    ...
    btn.addEventListener("click", function () {
        general_condtion_save(textarea, c_col_01);
    });
...
function general_condtion_save(nameObj, errorObj) {
    if (nameObj.value == "") {
    ...
    $.ajax({
        url: "create_new_condition/save_general_condition.php",
        type: 'POST',
        data: sending_value,
        cache: false,
        success: function (data) {
...
function general_del(id, div_row) {
    var sending_value = "id=" + id;
    $.ajax({
        url: "create_new_condition/del_general.php",
        type: 'POST',
        data: sending_value,
        cache: false,
        success: function (data) {
