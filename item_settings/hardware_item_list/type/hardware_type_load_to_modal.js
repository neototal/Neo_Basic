function select_item_type() {
    ...
    var add_item_type = document.createElement("button");
    add_item_type.setAttribute("class", "w3-theme-dark w3-button w3-input w3-hover-blue-grey w3-round");
    add_item_type.addEventListener("click", function () {
        add_new_item_type();
    });

    add_item_type.appendChild(document.createTextNode("add new"));
    a_col_02.appendChild(add_item_type);
...
function load_item_types(search_value, body_div) {
    ...
    $.ajax({
        url: "hardware_item_list/type/laod_data.php",
        type: 'POST',
        data: search_value,
        cache: false,
...
//            alert(data);
            ...
            for (var i = 0; i < json.length; i++) {
                load_item_type_to_table(json[i].idhardware_type, json[i].hardware_type, body_div);
            }
            if (json.length == 0) {
