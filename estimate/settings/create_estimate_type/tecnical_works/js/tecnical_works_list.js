function tecnical_work_list() {
    var header = document.getElementById("modal_head");
    $(header).empty();
    header.appendChild(document.createTextNode("Select technical works"));
    ...
    a_reload.addEventListener("click", function () {
        load_tecnical_works_list(this.value, b_container_body);
    });
    ...
    load_tecnical_works_list("", b_container_body);
...
function load_tecnical_works_list(search_value, body_div) {
    $(body_div).empty();
    var search_values = "search=" + search_value;
    $.ajax({
        url: "create_estimate_type/tecnical_works/select_tecnical_work_list.php",
        type: 'POST',
        data: search_value,
        cache: false,
...
function load_tecnical_works_to_table(id, name, body_div) {
    var a_div_row = document.createElement("div");
